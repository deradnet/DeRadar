/**
 * SVG to WebGL Texture Converter
 * Converts aircraft SVG icons to Pixi.js textures for GPU rendering
 */

// Aircraft shapes from lib/aircraft-icons.ts
...
// Color states for aircraft
const colors = {
  selected: '#00FF00',   // Green
  emergency: '#FF4444',  // Red
  highAlt: '#00AAFF',    // Blue
  highSpeed: '#FF8800',  // Orange
  ground: '#888888',     // Gray
  default: '#FFD700',    // Gold
};

/**
 * Convert SVG to Canvas, then to WebGL texture
 */
function svgToTexture(shapeKey, color, size = 48) {
  ...
  const scale = Math.min(size / vbW, size / vbH) * 0.8; // 0.8 for padding
  const offsetX = (size - vbW * scale) / 2 - vbX * scale;
  const offsetY = (size - vbH * scale) / 2 - vbY * scale;
 ...
 * Returns: { [shapeKey-colorKey]: canvas }
 */
function createAllTextures(size = 48) {
  const textures = {};

  Object.keys(aircraftShapes).forEach(shapeKey => {
    Object.entries(colors).forEach(([colorKey, colorValue]) => {
      const key = `${shapeKey}-${colorKey}`;
      textures[key] = svgToTexture(shapeKey, colorValue, size);
    });
  });

  console.log(`✓ Created ${Object.keys(textures).length} aircraft textures`);
  return textures;
}

/**
 * Get texture key for aircraft
 */
function getTextureKey(aircraft, isSelected = false) {
  // Determine shape
  ...
  if (isSelected) colorKey = 'selected';
  else if (isEmergency) colorKey = 'emergency';
  else if (isOnGround) colorKey = 'ground';
  else if ((aircraft.alt_baro || 0) > 35000) colorKey = 'highAlt';
  else if ((aircraft.gs || 0) > 400) colorKey = 'highSpeed';
...
if (typeof window !== 'undefined') {
  window.AircraftTextures = {
    createAllTextures,
    getTextureKey,
    shapes: Object.keys(aircraftShapes),
